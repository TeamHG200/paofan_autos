...
> 泡饭作者对饭否API做了精简抽象，提供了精简的脚本实现，只需要简单几行代码即可完成稀奇古怪的应用，稍微有点编程基础的都可以轻松上手。

> 服务器会把用户设计的机器人共享给全部用户，用户可以选择使用它来武装自己的账号，比如把自己变成一个大笨钟定时骚扰别人⏰，又或者是自动回复每一句晚安的小鲸鱼🐳。
...
| ff:home | table | 无 | 获取自己的时间线 | 
| ff:reply | table | 无 | 获取回复自己的消息 | 
| db:getScript | table | 无 | 获取当前用户的脚本参数 | 
| db:setScript | table | 无 | 设置当前用户的脚本参数 | 
